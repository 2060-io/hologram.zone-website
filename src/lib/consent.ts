 ...
 *
 * The key (`hologram-consent`) and the 12-month retention are documented
 * in the Cookie Policy at `/cookies` — keep the two in sync.
 *
...
export const STORAGE_KEY = "hologram-consent";
export const EVENT_NAME = "hologram:cookie-consent-change";
...
export function readStoredConsent(): ConsentValue | null {
  try {
    ...
    // quota exceeded). Treat as "undecided"; the banner has its own
    // guard so it does not spam the user on every page.
    return null;
  }
}
